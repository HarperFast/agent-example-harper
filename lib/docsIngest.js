...
export async function embedAndStore(DocChunk, source, chunk, index, ingestedAt) {
	...
	// Ingestion takes 30+s for the full corpus, often longer than the upstream
	// HTTP-proxy idle timeout. Detach from the request signal so a closed
	// connection doesn't kill in-flight embeds.
	const embedding = await embed(embedInput, { detachFromRequest: true });
	const row = {
		id,
		sourceUrl: source.url,
		title: chunk.title,
